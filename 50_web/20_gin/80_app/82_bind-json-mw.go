...
type productHandler2 struct {
	sync.RWMutex
	products map[string]Product2
}

func newProductHandler2() *productHandler2 {
	return &productHandler2{
		products: make(map[string]Product2),
	}
}

func (u *productHandler2) Create(c *gin.Context) {
	u.Lock()
	defer u.Unlock()
...
func (u *productHandler2) Get(c *gin.Context) {
	u.Lock()
	defer u.Unlock()
...
func router2() http.Handler {
	router := gin.Default()
	productHandler := newProductHandler2()
