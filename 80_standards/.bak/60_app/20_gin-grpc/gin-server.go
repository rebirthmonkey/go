...
func (server *GINServer) Run() error {
	...
	eg.Go(func() error {
		log.Infof("[GIN Server] Start to listening on http address: %s", server.insecureServerInfo.Address)
	...
	eg.Go(func() error {
		...
		//if cert == "" || key == "" || productInfoHandler.SecureServingInfo.BindPort == 0 {
		if cert == "" || key == ""  {
			return nil
		}

		log.Infof("[GIN Server] Start to listening on https address: %s", server.secureServerInfo.Address)

		//if err := productInfoHandler.secureServer.ListenAndServeTLS(cert, key); err != nil && !errors.Is(err, http.ErrServerClosed) {
	...
	// Ping the productInfoHandler to make sure the router is working.
	//ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	//defer cancel()
	//if productInfoHandler.healthz {
	//	if err := productInfoHandler.ping(ctx); err != nil {
	//		return err
	//	}
	//}
